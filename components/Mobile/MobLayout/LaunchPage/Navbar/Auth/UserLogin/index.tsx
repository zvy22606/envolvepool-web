      ...
      validator.validate(formData, async (errors, fields) => {
        if (!errors) {
          try {
            ...
            const toPageUrl = redirect_url ? `${redirect_url}?token=${res.token}` : '/';
            changeNavState();
            window.location.reload();
            // if (!redirect_url) window.location.reload();
    ...
    <div className="flex h-full w-full flex-col justify-between">
      ...
      <div className="flex w-full flex-col gap-4">
        <Button
          ...
          iconPosition="right"
          type="primary"
          className="button-text-m py-4 uppercase"
        >
          Continue
        </Button>
        <Button onClick={onBack} block ghost className="button-text-m py-4 uppercase">
          Back
        </Button>
      </div>
    </div>
